...
def hyp_plot_parameters(set_name, plotXFe, saveplot):
    ...
    # Start the process for plotting.
        plt.clf()
        ...
        # Start with a rectangular Figure and adjusted figure size for fitting within the PDF
        ...
        # Set the limits on the scatter plot (unchanged)
        ...
        axScatter.set_xlabel("[Fe/H]", fontsize=25)
        if plotXFe:
            if n == "Y1":
                axScatter.set_ylabel("[Y/Fe]", fontsize=25)
            else:
                axScatter.set_ylabel("[" + n + "/Fe]", fontsize=25)
        else:
            if n == "Y1":
                axScatter.set_ylabel("[Y/H]", fontsize=25)
            else:
                axScatter.set_ylabel("[" + n + "/H]", fontsize=25)

        axHistx.set_ylabel("Relative Dist", fontsize=18)
        axHisty.set_xlabel("Relative Dist", fontsize=18)

        # Increase tick label size
        axScatter.tick_params(axis='both', which='major', labelsize=15)
        axHistx.tick_params(axis='y', labelsize=12)
        axHisty.tick_params(axis='x', labelsize=12)

        # Adjust layout to ensure the plot fits within bounds
        plt.tight_layout()

        # Set legend location
        if n == "Mn" or not plotXFe:
            axScatter.legend(loc='upper left', scatterpoints=1, fontsize=14)
        else:
            axScatter.legend(loc='lower left', scatterpoints=1, fontsize=14)

        # Save or display the plot as needed
        if saveplot:
            full_plot_dir = os.path.join(working_dir, set_name, 'figures_3')
            if not os.path.isdir(full_plot_dir):
                os.mkdir(full_plot_dir)
            if plotXFe:
                plt.savefig(os.path.join(full_plot_dir, "predicted" + n + "Fe" + str(datetime.today().strftime('-%h%d-%H%M')) + ".pdf"))
            else:
